...
int main() {
	...
	//AES<ECB_Mode>::Encryption en128(AesKey("aaaabbbbccccdddd", 16));

	//en128.add("helloworld", 10);
	...
	//AES<ECB_Mode>::Decryption de128(AesKey("aaaabbbbccccdddd", 16));
	//de128.add(res);
...
//----
	...
	//AES<ECB_Mode>::Encryption en128(AesKey("1111222233334444", 16));
	...
	//cout << "程序用时=" << tt.count()/1000 << " MS" << endl;
	//cout << en128.end().size();
...
//----
	...
	//fopen_s(&fp_s, "F:\\game\\pic.zip", "rb");
	//fopen_s(&fp_d, "F:\\game\\pic.zip.aes", "wb");
	...
	//auto start = steady_clock::now();
	//while (1) {
	//	int n = fread(buffer, 1, 10240, fp_s);

	//	if (n != 10240) break;

	//	en128.add((char*)buffer, n);

	//	btring res = en128.end();

	//	fwrite(res.str(), 1, 10240, fp_d);
